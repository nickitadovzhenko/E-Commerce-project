...
def checkout(request):
    ...
    if request.user.is_authenticated:
        ...
        try:
            ...
            shippping_address = ShippingAddress.objects.get(
                user=request.user.id)

            context = {'shipping': shippping_address}
...
def complete_order(request):
    ...
    if request.POST.get('action') == 'post':
        ...
        if request.user.is_authenticated:
            ...
            order = Order(full_name=name, email=email, shipping_address=shipping_address,amount_paid=total_cost)
